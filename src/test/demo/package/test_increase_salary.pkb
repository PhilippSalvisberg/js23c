create or replace package body test_increase_salary is
   ...
   procedure increase_salary_jsloop_moderate is
      c_actual   sys_refcursor;
      c_expected sys_refcursor;
   begin
      -- act
      increase_salary_jsloop(in_deptno => 10, in_by_percent => 10, in_times => 2);
      ...
      open c_expected for
         ...
         select empno, ename, sal*1.1*1.1 as sal
           from original;
      ut.expect(c_actual).to_equal(c_expected);
   end increase_salary_jsloop_moderate;

   procedure increase_salary_jsloop_out_of_scale is
   begin
      -- act
      -- raises ORA-01438: value 100050 greater than specified precision (7, 2) for column 
      increase_salary_jsloop(in_deptno => 10, in_by_percent => 5000, in_times => 2);
   end increase_salary_jsloop_out_of_scale;
